...
class Endpoint:
    ...
    def validate(self):
        ...
        if (
            ...
            raise ProviderError(
                "Use an explicit HTTPS base URL, or opt into loopback HTTP."
            )
        ...
        if not self.model.strip() or not self.key_env.isidentifier():
            raise ProviderError(
                "A model ID and credential environment-variable name are required."
            )
        if (
    ...
    try:
        if dialect == "openai-chat":
            ...
            if choice.get("finish_reason") != "stop" or choice["message"].get(
                ...
                raise ProviderError(
                    "Response is incomplete or requires unsupported tools."
                )
            ...
            if any(item.get("type") not in ("message", "reasoning") for item in output):
                raise ProviderError(
                    "Response requires an unsupported tool or output type."
                )
            text = "".join(
                part["text"]
                for item in output
            ...
            if response.get("stop_reason") != "end_turn" or any(
                part.get("type") != "text" for part in response["content"]
            ):
...
def complete(config, prompt, system=""):
    ...
    try:
        ...
        exc.close()
        raise ProviderError(
            f"Provider HTTP {code}; no automatic retry or fallback."
        ) from None
    except (URLError, TimeoutError):
        raise ProviderError(
            "Provider transport failed; billing/outcome may be unknown."
        ) from None
