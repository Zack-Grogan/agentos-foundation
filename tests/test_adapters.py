...
from adapters.http_models import Endpoint, ProviderError, build_request, parse_response, complete


class EndpointTests(unittest.TestCase):
    def config(self, dialect):
        return Endpoint('https://provider.example/v1', dialect, 'fixture-model', 'AGENTOS_TEST_KEY')

    @patch.dict(os.environ, {'AGENTOS_TEST_KEY': 'fixture-not-a-real-key'})
    def test_three_explicit_request_dialects(self):
        for dialect, path in [('openai-chat', '/chat/completions'), ('openai-responses', '/responses'), ('anthropic-messages', '/messages')]:
            r = build_request(self.config(dialect), 'Selected text', 'System instruction')
            ...
            if dialect == 'anthropic-messages':
                ...
                self.assertEqual(r.get_header('Authorization'), 'Bearer fixture-not-a-real-key')
    ...
    def test_auth_url_and_dialect_validation(self):
        ...
        for url in ['http://remote.example/v1', 'https://user:password@example.com/v1', 'https://example.com/v1?key=bad']:
    ...
    def test_output_normalization_and_incomplete(self):
        responses = {
            'openai-chat': {'choices': [{'finish_reason': 'stop', 'message': {'content': 'Hello'}}]},
            'openai-responses': {'status': 'completed', 'output': [{'type': 'message', 'content': [{'type': 'output_text', 'text': 'Hello'}]}]},
            'anthropic-messages': {'stop_reason': 'end_turn', 'content': [{'type': 'text', 'text': 'Hello'}]}}
        ...
        with self.assertRaises(ProviderError):
            parse_response('openai-chat', {'choices': [{'finish_reason': 'length', 'message': {'content': 'partial'}}]})
        with self.assertRaises(ProviderError):
            parse_response('anthropic-messages', {'stop_reason': 'tool_use', 'content': []})
    ...
    def test_real_http_peer_and_redirect_refusal(self):
        ...
        class Peer(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                ...
                response = {'choices': [{'finish_reason': 'stop', 'message': {'content': 'Fixture output'}}]}
        ...
        try:
            ...
            c = Endpoint(base+'/v1', 'openai-chat', 'fixture', 'AGENTOS_TEST_KEY', allow_loopback_http=True)
            self.assertEqual(complete(c, 'Hello')['text'], 'Fixture output')
            with self.assertRaisesRegex(ProviderError, '307'):
                complete(Endpoint(base+'/redirect', 'openai-chat', 'fixture', 'AGENTOS_TEST_KEY', allow_loopback_http=True), 'Hello')
...
class ACPTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.client = ACPClient([sys.executable, str(Path(__file__).with_name('fake_acp.py').resolve())], cwd=self.tmp.name, environment={}, timeout=1)
    ...
    async def test_negotiate_prompt_updates_and_deny_permission(self):
        ...
        self.assertEqual(self.client.updates[0]['update']['content']['text'], 'Permission denied; fixture response.')
