...
class CoreTests(unittest.TestCase):
    ...
    def source(self):
        return self.store.capture(
            ...
            "Compare two proposed methods using cited evidence.",
            "capture-1",
        )
    ...
    def test_complete_loop_and_restart(self):
        ...
        self.assertTrue(
            all(t["status"] == "proposed" for t in state["projects"][0]["tasks"])
        )
    ...
    def test_competing_acceptances_create_one_project(self):
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            results = list(
                pool.map(
                    lambda _: self.store.review(a["id"], a["digest"], "accepted"),
                    range(2),
                )
            )
